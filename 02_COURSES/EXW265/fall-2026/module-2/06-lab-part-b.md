<!--
  COURSE: EXW 265 Exercise for Special Populations
  MODULE 2: Type 2 Diabetes (Metabolic)
  COMPONENT: Lab Part B (The Solve)
  THEME: Creme (9:1 High Contrast)
  ACCESSIBILITY: WCAG 2.1 AA Compliant
-->

<div class="exw-container"
    style="background-color: #fff9f0; color: #000080; padding: 40px; font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; border: 1px solid #d4d0b8; max-width: 900px; margin: auto;">

    <!-- HEADER -->
    <header style="border-bottom: 3px solid #000080; margin-bottom: 30px; padding-bottom: 10px;">
        <h2 style="color: #000080; margin: 0; font-size: 1.8rem; text-transform: uppercase; letter-spacing: 1px;">
            06: Lab 02 - The Metabolic Solve
        </h2>
        <p style="font-weight: bold; color: #000080;">Mission Loop: Post-Prandial Regulation</p>
    </header>

    <section>
        <h3 style="color: #000080; border-left: 5px solid #000080; padding-left: 15px;">The Solve</h3>
        <p>Your mission is to design a movement plan for a T2D client that emphasizes timing. Using the <strong>Einstein Protocol</strong>, you must justify why 10-20 minute sessions <strong>after meals</strong> are your primary clinical tactic.</p>
    </section>

    <section id="submission-protocol">
        <h3 style="color: #000080;">Submission Protocol</h3>
        <p><strong>Type: Text-Only</strong></p>
        <ol>
            <li><strong>Presence Defense:</strong> Defend the use of post-prandial walking for glucose management.</li>
            <li><strong>The Plan:</strong> Weekly schedule (150 mins aerobic + 2 days resistance).</li>
            <li><strong>AI Audit:</strong> Comparison against PAGA 2018.</li>
        </ol>
        <p style="background-color: #ba0c2f; color: #FFFDD0; padding: 10px; font-weight: bold; text-align: center;">REDLINE: PHYSICIAN CLEARANCE IS NOT REQUIRED FOR LIGHT WALKING.</p>
    </section>

    <!-- NAVIGATION -->
    <nav style="display: flex; justify-content: center; gap: 15px; margin-bottom: 30px; margin-top: 60px;">
        <a href="/courses/38156/modules" style="background-color: #FFFDD0; color: #000080; border: 2px solid #000080; padding: 14px 28px; text-decoration: none; font-weight: bold; border-radius: 4px; text-align: center; min-width: 120px;">MODULES</a>
    </nav>
</div>

---
**Author:** Coach Alan Pruitt  
**Version:** 1.0 (Fall 2026 Prep)  
**License:** CC BY-NC 4.0

**SSoT:** PAGA 2018 (2nd Ed)
